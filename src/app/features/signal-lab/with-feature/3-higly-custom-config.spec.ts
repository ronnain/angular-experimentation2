...
describe('withBooksFilter3', () => {
  it('should filter books by query', () => {
    const BooksStore = signalStore(
      ...
      withBooksFilter3((store) => booksSelector({ booksPath: 'entities' })), // 👉 autocomplete: 'entities' | 'topBooks'
      withHooks((store) => ({
        onInit: () => {
          patchState(
            store,
  ...
  it('Should be typed', () => {
    const BooksStore = signalStore(
      ...
      withBooksFilter3((store) => booksSelector({ booksPath: 'entities' })),
      withBooksFilter3((store) =>
        //@ts-expect-error
        booksSelector({ booksPath: 'errorNotExistingPath' })
      )
    );
